...
description: Orchestrate card image generation using the Gemini-powered gen-image-gemini script — find missing images, craft prompts from card data, and iteratively refine results.
---
...
```bash
cd packages/web && npm run gen-image-gemini -- missing
```
...
```bash
cd packages/web && npm run gen-image-gemini -- describe <card-id> "<description>"
```

If an image or task already exists and the user wants to start over:

```bash
cd packages/web && npm run gen-image-gemini -- describe <card-id> "<description>" --redo
```
...
```bash
cd packages/web && npm run gen-image-gemini -- amend <card-id> "<what to change>"
```
...
```bash
cd packages/web && npm run gen-image-gemini -- status
```
