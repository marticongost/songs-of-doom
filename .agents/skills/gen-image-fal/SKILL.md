---
name: gen-image-fal
description: Orchestrate card image generation using the fal.ai-powered gen-image-fal script — find missing images, craft prompts from card data, and iteratively refine results.
---

# Generate card images with fal.ai

## When to use this skill

Use this skill when the user wants to generate, regenerate, or refine artwork for one or more cards using fal.ai. Typical triggers:

- "Generate an image for \<card\>"
- "What cards are missing images?"
- "Redo the warrior image (use fal)"
- "The sword image looks wrong, make it darker"

## Setup check

All commands run from `packages/web/`. The script requires `FAL_KEY` in `.env`.
...
```bash
cd packages/web && npm run gen-image-fal -- missing
```
...
```bash
cd packages/web && npm run gen-image-fal -- describe <card-id> "<description>"
```

If an image or task already exists and the user wants to start over:

```bash
cd packages/web && npm run gen-image-fal -- describe <card-id> "<description>" --redo
```
...
```bash
cd packages/web && npm run gen-image-fal -- amend <card-id> "<what to change>"
```

`amend` appends the amendment to the original description and generates a new image. Unlike the Gemini version, fal.ai does not use the previous image as a reference — each generation is independent.
...
```bash
cd packages/web && npm run gen-image-fal -- status
```
...
- The state file (`packages/web/scripts/.image-gen-fal-state.json`) persists across sessions. A card's full amendment history is stored there.
